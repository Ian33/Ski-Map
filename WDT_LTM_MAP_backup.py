...
def site_import(file_path, parameter=None):
    ...
    sites = sites[[
        ...
        "KM notes", "annual equipment cost", "date installed"
    ]]
...
def add_map_legend(m, layer_name='Sites by Parameter', show=True):
    """Add legend to map"""
    legend_html = f'''
    <div id="parameter-legend" style="
        ...
        <h4 style="margin: 0 0 10px 0; font-size: 16px;">New Sites</h4>
        <div style="margin: 5px 0;">
            <span style="display: inline-block; width: 12px; height: 12px; 
                         background-color: #00A5E3; border-radius: 50%; margin-right: 8px;"></span>
            Discharge
        </div>
        <div style="margin: 5px 0;">
            <span style="display: inline-block; width: 12px; height: 12px; 
                         background-color: #E77577; border-radius: 50%; margin-right: 8px;"></span>
            Water Temperature
        </div>
        <div style="margin: 5px 0;">
            <span style="display: inline-block; width: 12px; height: 12px; 
                         background-color: #8DD7BF; border-radius: 50%; margin-right: 8px;"></span>
            Precipitation
        </div>
        <hr style="border: none; border-top: 3px solid #74737A; margin: 5px 0;">
        <div style="margin: 5px 0;">
            <span style="display: inline-block; width: 12px; height: 12px; 
                         background-color: #969696; border-radius: 50%; margin-right: 8px;"></span>
            Site/basin outside WTD service area
        </div>
        <hr style="border: none; border-top: 3px solid #74737A; margin: 5px 0;">
        <div style="margin: 5px 0;">
...
def add_filtered_map_legend(m, layer_name='Sites by Parameter', show=True):
    """Add legend to map"""
    legend_html = f'''
    <div id="parameter-legend" style="
        position: fixed; bottom: 10px; left: 10px; width: 200px;
    ...
    m.get_root().html.add_child(folium.Element(legend_html))
    return m
def add_filtered_isp_map_legend(m, layer_name='Sites by Parameter', show=True):
    """Add legend to map"""
    ...
    <div id="parameter-legend" style="
        ...
        <div style="margin: 5px 0;">
            <span style="display: inline-block; width: 12px; height: 12px; 
                         background-color: #D53E4F; border-radius: 50%; margin-right: 8px;"></span>
            ISP Sites
        </div>
        <div style="margin: 5px 0;">
            <span style="display: inline-block; width: 12px; height: 12px; 
                         background-color: grey; border-radius: 50%; margin-right: 8px;"></span>
            Non-ISP Sites
        </div>
...
def add_filtered_sites(m, sites_gdf, parameter_filter=None, program_filter=None, 
    ...
    if program_filter is not None:
        ...
        filtered_sites = filtered_sites[filtered_sites['program'].isin(program_filter)]
...
def create_map(sites_gdf, wtd_service_area, wtd_basins):
    ...
    if wtd_basins is not None and not wtd_basins.empty:
        wtd_basin_layer = folium.FeatureGroup(name='WTD Basins', show=True)
        folium.GeoJson(
            wtd_basins,
            style_function=lambda x: {
                'fillColor': '#20B2AA',
    ...
    # Add legend
    add_map_legend(m, layer_name='Sites by Parameter', show=True)
...
def create_filtered_map(sites_gdf, wtd_service_area, wtd_basins):
    """Create Folium map with sites and WTD service area"""
    # filter out non wtd sites
    wtd_sites = sites_gdf[sites_gdf["WTD Service Area"] == True]
    # Center map on sites
    bounds = wtd_sites.total_bounds
    center_lat = (bounds[1] + bounds[3]) / 2
    center_lon = (bounds[0] + bounds[2]) / 2
    ...
    # Add legend
    add_filtered_map_legend(m, layer_name='Sites by Parameter', show=True)
...
def create_filtered_isp_map(sites_gdf, wtd_service_area, wtd_basins):
    """Create Folium map with sites and WTD service area"""
    # filter out non wtd sites
    wtd_sites = sites_gdf[sites_gdf["WTD Service Area"] == True]
    # Center map on sites
    ...
    add_filtered_sites(m, wtd_sites, parameter_filter=None, program_filter=None, 
                       exclude_empty_notes=False, layer_name='Non ISP Sites', 
                       color='#D53E4F', show=True, radius=5)
    # isp = '#fee08b' or blue #8bc9fe non isp = '#D53E4F'
    #non_wtd_sites = sites_gdf[sites_gdf["WTD Service Area"] == False]
    #add_filtered_sites(m, non_wtd_sites, layer_name='Non WTD Sites', color='#969696', show=True, radius=6)
    
    # Add legend
    add_filtered_isp_map_legend(m, layer_name='Sites by Parameter', show=True)
...
if __name__ == "__main__":
    ...
    m.save("WTD_map/data/wtd_map_all.html")
    ...
    output_cols = [
        ...
        "Yearly Hours", "KM verified", "KM notes", "annual equipment cost"
    ]
    ...
    basins_filter = None
    # Create filtered and save map
    m = create_filtered_map(sites_gdf, wtd_service_area, basins_filter)
    m.save("WTD_map/data/wtd_map_filtered.html")

    # Save screenshot
    save_map_screenshot(
        html_path='WTD_map/data/wtd_map_filtered.html',
        output_path='WTD_map/data/wtd_map_filtered.png',
        pdf_path='WTD_map/data/wtd_map_filtered.pdf',
        window_size=(729, 943)
    )

    ### create filterd isp map
    m = create_filtered_isp_map(sites_gdf, wtd_service_area, basins_filter)
    m.save("WTD_map/data/wtd_map_filtered_isp.html")

    # Save screenshot
    save_map_screenshot(
        html_path='WTD_map/data/wtd_map_filtered_isp.html',
        output_path='WTD_map/data/wtd_map_filtered_isp.png',
        pdf_path='WTD_map/data/wtd_map_filtered_isp.pdf',
        window_size=(729, 943)
    )
